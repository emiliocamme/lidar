...
class KalmanFilter:
    def __init__(self, process_noise=0.01, measurement_noise=0.099, estimate_error=0.2):
        # State covariance
    ...
    def predict(self, u):
        """Prediction step: Update the state estimate and error covariance."""
        self.x += u  # Predict the next state
        self.P += self.Q  # Update the error covariance

    def update(self, z):
        """
        Update step: Incorporate the measurement.
        :param z: Measurement of likelihood.
        """
        # Kalman Gain
    ...
    def get_obstacle_likelihood(self):
        """
        Returns the current likelihood estimate.
        """
        return self.x
...
class LidarPublisher(Node):
    ...
    def navigate_based_on_map(self):
        ...
        rotation_step = 0.0225  # Step size for rotation in radians
    ...
    def check_obstacle_combined(self, target_x, target_y):
        ...
        # Fuse measurements using the KF
        self.kf.predict(0)#scalar


        self.kf.update(map_based_obstacle or lidar_based_obstacle) #scalar

        
        self.get_logger().info(f"Likelihood {self.kf.get_obstacle_likelihood()}") #scalar
        
        
        #scalar
        return self.kf.get_obstacle_likelihood() > 0.2
