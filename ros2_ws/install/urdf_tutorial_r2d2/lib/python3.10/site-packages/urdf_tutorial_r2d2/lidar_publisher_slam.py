...
class LidarPublisher(Node):
    def __init__(self):
        ...
        # Movement parameters
        self.linear_velocity = 0.001  # m/s
        self.angular_velocity = math.radians(0.5)  # rad/s
        self.position_x = 0.0
        self.position_y = 0.0
        self.yaw = 0.0

    def timer_callback(self):
        # You can modify the position directly here or from another source.
        # For example, you might read the desired position from a parameter or another input.
        self.position_x += 0.001 * math.cos(self.yaw)  # Move in x direction
        self.position_y += 0.001 * math.sin(self.yaw)  # Move in y direction
        self.yaw = math.atan2(self.position_y, self.position_x)


        # Broadcast transformation for ROS2 TFs
        self.broadcast_base_link_transform(self.position_x, self.position_y, self.yaw)
